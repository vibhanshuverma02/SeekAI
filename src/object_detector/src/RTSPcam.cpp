...
void RTSPcam::Open(const string& MyString, int apiPreference)
{
    ...
    UsePicture = false;
    UseFolder  = false;              
       // true when only the folder name is loaded.
//In a program with multi-threading, reading the next frame and processing it are not //required to b
...
bool RTSPcam::GetLatestFrame(cv::Mat& frame)
{
    ...
    if(!FirstPic){
        ...
        if(Elapse > FrameTime){
            LostFrames=ceil(1.05*Elapse/FrameTime);     //1.15 to be sure you always have the latest frame, in case FrameTime isn't accurate
