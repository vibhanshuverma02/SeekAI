...
namespace yolov5 {
 ...
 RTSPReader::RTSPReader(const std::string& rtsp_url, float confidence_threshold, float iou_threshold)
    : rtsp_url(rtsp_url), stopped(false), confidence_threshold(confidence_threshold), iou_threshold(iou_threshold),detector(detector) {}   
    ...
    void RTSPReader::start(){
        ...
        // Start the threads
        capture_thread = std::thread(&RTSPReader::captureLoop, this);
        inference_thread = std::thread(&RTSPReader::inferenceLoop, this);
        gstreamer_thread = std::thread(&RTSPReader::gstreamerLoop, this);
    }
    void RTSPReader::stop() {
        ...
        gstreamer_cv.notify_all();

        if (capture_thread.joinable()) capture_thread.join();
        if (inference_thread.joinable()) inference_thread.join();
        if (gstreamer_thread.joinable()) gstreamer_thread.join();

        cap.release();
    } 

void RTSPReader::captureLoop (){
     while (!stopped) {
        cv::Mat new_frame; 
        cap.read(new_frame);
        if (!new_frame.empty()) {
...
void RTSPReader::inferenceLoop(){
    while(!stopped){
       std::unique_lock<std::mutex> lock(mutex);
            inference_cv.wait(lock);

          detector.Inference(frame,confidence_threshold, iou_threshold); 
             inferred_frame = frame.clone();

            gstreamer_cv.notify_one();
    }
}
void RTSPReader:: gstreamerLoop() {
        // Initialize GStreamer pipeline
        gst_init(NULL, NULL);

        GMainLoop* loop = g_main_loop_new(NULL, FALSE);

        std::string pipeline_desc = "appsrc name=appsrc ! videoconvert ! queue ! autovideosink";
        GError* error = nullptr;
        GstElement* pipeline = gst_parse_launch(pipeline_desc.c_str(), &error);
        if (!pipeline) {
            std::cerr << "Error creating pipeline: " << error->message << "\n";
            if (error) {
                g_error_free(error);
            }
            return;
        ...
        if (!appsrc) {
            std::cerr << "Error getting appsrc element\n";
        ...
        if (gst_element_set_state(pipeline, GST_STATE_PLAYING) == GST_STATE_CHANGE_FAILURE) {
            std::cerr << "Error starting pipeline\n";
            gst_object_unref(pipeline);
            return;
        }
        ...
        while (!stopped) {
            std::unique_lock<std::mutex> lock(mutex);
            gstreamer_cv.wait(lock);

            // Push 'inferred_frame' into GStreamer pipeline
            if (!inferred_frame.empty()) {
                GstBuffer* buffer = gst_buffer_new_allocate(nullptr, inferred_frame.total() * inferred_frame.elemSize(), nullptr);
                gst_buffer_fill(buffer, 0, inferred_frame.data, inferred_frame.total() * inferred_frame.elemSize());
                ...
                if (ret != GST_FLOW_OK) {
                    std::cerr << "Error pushing buffer to pipeline\n";
                    break;
                }
            }
...
void ObjectDetector::Inference(const cv::Mat& input_image,float confidence_threshold, float iou_threshold) {
  std::cout << "=== Empty inferences to warm up === \n\n";
    ...
    SaveResultImage(input_image, results);
